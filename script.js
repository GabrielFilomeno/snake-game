let canvas = document.getElementById("snakeHead"); // criar elemento que irá rodar o jogo
let context = canvas.getContext("2d");
let box = 32;

let snakeHead = []; // criar cobrinha como lista, já que ela vai ser uma série de coordenadas, que quando pintadas, criam os quadradinhos
snakeHead[0] = {
    x: 8 * box,
    y: 8 * box
};
...
function startGame() {
    ...
    if (snakeHeadX != food.x || snakeHeadY != food.y) {
        snakeHead.pop(); // pop tira o último elemento da lista
    } else {
    ...
    snakeHead.unshift(newHead); // método unshift adiciona o primeiro quadrado da cobrinha
